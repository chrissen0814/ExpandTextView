...
public class MoonUtil {
    ...
    private static final float DEFAULT_EMOJI_SCALE_VALUES = 0.35f;
    ...
    public static SpannableString replaceEmoticons(Context context, String value, float scale, int align) {
        ...
        while (matcher.find()) {
            ...
            Drawable d = getEmotDrawable(context, emot, DEFAULT_EMOJI_SCALE_VALUES);
            if (d != null) {
                JHImageSpan span = new JHImageSpan(d, align);
    ...
    public static SpannableString replaceEmoticonsBySize(Context context, String value, int fontSize, int align) {
        if (TextUtils.isEmpty(value)) {
            value = "";
        }
        ...
        while (matcher.find()) {
            ...
            int end = matcher.end();
            String emot = value.substring(start, end);
            Drawable d = getEmotDrawableBySize(context, emot, fontSize);
            if (d != null) {
                JHImageSpan span = new JHImageSpan(d, align);
                mSpannableString.setSpan(span, start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
     ...
     *
     * @param context  上下文
     * @param editable 当前文本内容
     * @param start    表情开始位置索引
     * @param count    表情所占长度
     * @param textSize 表情的缩放比
     */
    public static void replaceEmoticons(Context context, Editable editable, int start, int count, int textSize) {
        ...
        while (matcher.find()) {
            ...
            int to = start + matcher.end();
            String emot = editable.subSequence(from, to).toString();
            Drawable d = getEmotDrawableBySize(context, emot, textSize);
            if (d != null) {
                JHImageSpan span = new JHImageSpan(d, ImageSpan.ALIGN_BOTTOM);
                editable.setSpan(span, from, to, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
    ...
    private static Drawable getEmotDrawableBySize(Context context, String text, int size) {
        Drawable drawable = EmojiManager.getDrawable(context, text);
